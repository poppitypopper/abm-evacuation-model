...
to setup
  ...
  set drawing? false
  set start-selected? false
  set end-selected? false
end

to draw-barriers
  if mouse-down? [
    ; Get all patches in a small radius around the mouse pointer for smoother drawing
    let nearby-patches patches with [
      distancexy mouse-xcor mouse-ycor < 0.7
    ]

    ; First click sets start point
    ifelse not start-selected? [
      ask patches with [pcolor = green] [set pcolor white]
      ask min-one-of nearby-patches [distancexy mouse-xcor mouse-ycor] [
        set pcolor green
        set start-patch self
      ]
      set start-selected? true
    ]
    ; Second click sets end point
    [ ifelse not end-selected? [
        let target-patch min-one-of nearby-patches [distancexy mouse-xcor mouse-ycor]
        if target-patch != start-patch [
          ask patches with [pcolor = red] [set pcolor white]
          ask target-patch [
            set pcolor red
            set end-patch self
          ]
          set end-selected? true
        ]
      ]
      ; After both points are set, draw barriers
      [
        ask nearby-patches [
          if self != start-patch and self != end-patch [
            set pcolor black
          ]
        ]
      ]
...
to reset-points
  ...
  set start-patch nobody
  set end-patch nobody
end

to find-path
  if not start-selected? or not end-selected? [
  ...
  ; Clean up the pathfinding turtle
  ask turtles [ die ]
end
@#$#@#$#@
GRAPHICS-WINDOW
